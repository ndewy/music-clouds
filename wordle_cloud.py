...
TIME_PERIODS= {"All Time":pylast.PERIOD_OVERALL,"Last Week":pylast.PERIOD_7DAYS,
    "Last Month":pylast.PERIOD_1MONTH,"Last 3 Months":pylast.PERIOD_3MONTHS,
    "Last 6 months":pylast.PERIOD_6MONTHS,"Last Year":pylast.PERIOD_12MONTHS}
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        limit_input = tk.Entry(self,textvariable=self.options["limit"],validatecommand=(self.validate,'%S'),validate='key')
    ...
    def generate(self):
        ...
        t = threading.Thread(target=generate, args=(self.on_generated,self.on_generating_fail,params))
        t.start()

    def on_generated(self,image_path):
        window = ImageWindow(image_path,master=self)
...
def generate(callback_success,callback_fail,parameters):
    ...
    try:
        artists_raw = network.get_user(parameters["user"]).get_top_artists(period=parameters["time"],limit=parameters["limit"])
